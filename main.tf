...
locals {
  ...
  create_custom_domain = (length(local.projects_need_api) > 0) && (local.hosted_zone_provided || local.domain_provided)
  ...
  api_base_url    = (local.create_custom_domain) ? local.custom_api_url : aws_api_gateway_stage.prod[0].invoke_url
